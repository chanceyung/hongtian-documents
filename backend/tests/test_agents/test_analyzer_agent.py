...
class TestAnalyzerAgentClusterContent:
    ...
    @pytest.mark.asyncio
    async def test_cluster_content_calls_llm(self, analyzer_agent, sample_document):
        """Test that _cluster_content calls LLM."""
        analyzer_agent.llm.chat_json_list = AsyncMock(return_value=[
            {"group_id": "group-1", "theme": "Section 1", "text_ids": ["text-1", "text-2"], "image_ids": [], "table_ids": [], "suggested_layout": "text_only"}
        ])

        result = await analyzer_agent._cluster_content(sample_document)

        analyzer_agent.llm.chat_json_list.assert_called_once()
        assert len(result) > 0
    ...
    async def test_cluster_content_preserves_ids(self, analyzer_agent, sample_document):
        ...
        analyzer_agent.llm.chat_json_list = AsyncMock(return_value=[
            {"group_id": "group-1", "theme": "Intro", "text_ids": ["text-1", "text-2"], "image_ids": [], "table_ids": [], "suggested_layout": "text_only"}
        ])
...
class TestAnalyzerAgentExtractPatterns:
    ...
    async def test_extract_patterns_truncates_long_text(self, analyzer_agent):
        ...
        analyzer_agent.llm.chat_json = AsyncMock(return_value={
            "document_type": "report", "target_audience": "general",
            "key_sections": [], "highlights": [], "suggested_pages": 1, "suggested_style": "modern"
        })
        result = await analyzer_agent._extract_patterns(doc)

        analyzer_agent.llm.chat_json.assert_called_once()
    ...
    async def test_extract_patterns_with_short_text(self, analyzer_agent, sample_document):
        ...
        analyzer_agent.llm.chat_json.assert_called()
...
class TestAnalyzerAgentSemanticLinkage:
    ...
    async def test_semantic_linkage_skips_linked_text(self, analyzer_agent):
        ...
        analyzer_agent.llm.chat_json_list = AsyncMock(return_value=[
            {"text_id": "text-1", "image_id": "img-1", "reason": "semantic match", "confidence": 0.9}
        ])
